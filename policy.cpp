...
policy::policy(unsigned n, unsigned p, unsigned k, unsigned d, matrix<double> feature, matrix<double> reward, unsigned int level)
{
  ...
  for (unsigned int i = 0; i < this->p; i++)
    ...
    //Get index of order according to each attribute..
    for (unsigned int i = 0; i < this->p; i++)
    {
...
void policy::learn_from_data(treenode * node)
{
  ...
  if (node->id == 1)
    ...
    // Transfer all data to right child and then tranfer one from right to left each loop
    for (unsigned int i = 0; i < this->p; i++)
    {
      for (unsigned int j = 0; j < this->n; j++)
	{
      ...
      { //Data's not in this sub tree...
        ...
        if (b % this->d == 0)
        { //Special conditon of duplicate feature..
...
void policy::leaf_learning(treenode * node)
{
  ...
  for (unsigned int i = 0; i < this->p; i++) {
    // Put data in this leaf node into a new set..
...
void policy::leaf_learning_jump(treenode *node)
{
  node->subtree_reward = 0;
  if (node->left == NULL)
  {
  ...
  for (unsigned int i = 0; i < this->p; i++)
  {
    ...
    if (d - 1 < label_num)
      start = d - 1; // jump
    for (int b = start; b <= label_num; b += d)
    { // jump
      node->left->subtree_reward = 0;
      node->right->subtree_reward = 0;
      for (int kk = 0; kk < this->k; kk++)
...
void policy::learn_from_data_leafjump(treenode *node)
{
  // If leaf node, call leaf learning...
  if (node->id >= (unsigned int)std::pow(2, (this->l - 1)))
  {
    leaf_learning_jump(node);
    return;
  }
  node->subtree_reward = 0;
  if (node->left == NULL)
